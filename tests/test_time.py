...
datetimes = [
    datetime.strptime(dt_str, "%Y-%m-%d %H:%M:%S") for dt_str in datetimes_strings
]
...
class TestTime(unittest.TestCase):
    ...
    def test_gps_time_week_rollover(self):
        for gps_time in gps_times:
            ...
            np.testing.assert_allclose(
                gps_time_plus_week.tow, gps_time.tow, rtol=0, atol=1e-10
            )
            np.testing.assert_allclose(
                gps_time_plus_week.week - 1, gps_time.week, rtol=0, atol=1e-10
            )
            np.testing.assert_allclose(
                gps_time_minus_week.tow, gps_time.tow, rtol=0, atol=1e-10
            )
            np.testing.assert_allclose(
                gps_time_minus_week.week + 1, gps_time.week, rtol=0, atol=1e-10
            )
    ...
    def test_gps_time_subtraction_addition(self):
        for gps_time in gps_times:
            ...
            delta_sec_plus_dt = (
                gps_time.as_datetime() - gps_time_plus_secs.as_datetime()
            ).total_seconds()
            delta_sec_minus_gps = gps_time - gps_time_minus_secs
            delta_sec_minus_dt = (
                gps_time.as_datetime() - gps_time_minus_secs.as_datetime()
            ).total_seconds()
            np.testing.assert_allclose(
                delta_sec_plus_dt, delta_sec_plus_gps, rtol=0, atol=1e-10
            )
            np.testing.assert_allclose(
                delta_sec_minus_dt, delta_sec_minus_gps, rtol=0, atol=1e-10
            )
    ...
    def test_utc_converter(self):
        ...
        gps_times = [
            GPSTime.from_datetime(datetime.strptime(dt_str, "%Y-%m-%d %H:%M:%S"))
            for dt_str in datetimes_strings
        ]
        np.testing.assert_allclose(
            (gps_times[0] - gpst_to_utc(gps_times[0])), 14, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[1] - gpst_to_utc(gps_times[1])), 15, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[2] - gpst_to_utc(gps_times[2])), 16, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[3] - gpst_to_utc(gps_times[3])), 17, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[4] - gpst_to_utc(gps_times[4])), 18, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[5] - gpst_to_utc(gps_times[5])), 18, rtol=0, atol=1e-3
        )
        np.testing.assert_allclose(
            (gps_times[6] - gpst_to_utc(gps_times[6])), 17, rtol=0, atol=1e-3
        )
