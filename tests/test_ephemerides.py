...
gps_times_list = [[1999, 415621.0], [2045, 455457.0], [1985, 443787.0]]
...
class TestAstroDog(unittest.TestCase):
        ...
        for gps_time in gps_times:
            for svId in svIds:
                ...
                np.testing.assert_allclose(
                    sat_info_nav[0], sat_info_orbit[0], rtol=0, atol=5
                )
                np.testing.assert_allclose(
                    sat_info_nav[1], sat_info_orbit[1], rtol=0, atol=0.1
                )
                np.testing.assert_allclose(
                    sat_info_nav[2], sat_info_orbit[2], rtol=0, atol=1e-7
                )
                np.testing.assert_allclose(
                    sat_info_nav[3], sat_info_orbit[3], rtol=0, atol=1e-11
                )
    ...
    def test_read_prn_data(self):
        ...
        def get_single_measurement(t):
            return [
                (EphemerisType.ULTRA_RAPID_ORBIT, GPSTime(0, 0), "filename.filetype"),
                GPSTime(week=2177, tow=172800.0 + t),
                -22481344.405,
                -14485178.376,
                -554329.557,
                0.000555129133,
