...
from kalman.helpers.sympy_helpers import sympy_into_c
from kalman.helpers import TEMPLATE_DIR, load_code, write_code
from kalman.helpers.chi2_lookup import chi2_ppf
...
def gen_code(
    ...
    for h_sym, kind, ea_sym, H_sym, He_sym in obs_eqs:
        ...
        maha_thresh = chi2_ppf(
            0.95, int(h_sym.shape[0])
...
class EKF_sym:
    def __init__(
        ...
        def wrap_1lists(name):
            ...
            def ret(lst1, out):
                func(
                    ffi.cast("double *", lst1.ctypes.data),
    ...
    def predict_and_update_batch(self, t, kind, z, R, extra_args=[[]], augment=False):
        ...
        if self.filter_time is not None and t < self.filter_time:
            if (
                len(self.rewind_t) == 0
                or t < self.rewind_t[0]
                or t < self.rewind_t[-1] - 1.0
            ):
                print(
                    f"observation too old at {t:.3f} with filter at {self.filter_time:.3f}, ignoring"
                )
                return None
    ...
    def _predict_and_update_batch(self, t, kind, z, R, extra_args, augment=False):
        ...
        for i in range(len(z)):
            ...
            # update
            self.x, self.P, y_i = self._update(
                self.x, self.P, kind, z_i, R_i, extra_args=extra_args_i
            )
            y.append(y_i)
    ...
    def rts_smooth(self, estimates, norm_quats=False):
        ...
        for k in range(len(estimates) - 2, -1, -1):
            ...
            d2 = self.dim_main_err
            Ck = np.linalg.solve(
                Pk1_k[:d2, :d2], Fk_1[:d2, :d2].dot(Pk_k[:d2, :d2].T)
            ).T
            xk_n = xk_k
            ...
            xk_n[:d1] = x_new[:d1, 0]
            Pk_n = Pk_k
            Pk_n[:d2, :d2] = Pk_k[:d2, :d2] + Ck.dot(
                Pk1_n[:d2, :d2] - Pk1_k[:d2, :d2]
            ).dot(Ck.T)
