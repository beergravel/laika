...
def generate_residual(K):
    ...
    sympy_functions.append(
        (
            "jac_fun",
            img_pos_residual_sym.jacobian(x_sym),
...
class LstSqComputer:
    ...
    def __init__(self, K=4, MIN_DEPTH=2, MAX_DEPTH=500):
        ...
        def compute_pos_c(poses, img_positions):
            ...
            lib.compute_pos(
                ffi.cast("double *", self.to_c.ctypes.data),
                ffi.cast("double *", poses.ctypes.data),
                ffi.cast("double *", img_positions.ctypes.data),
    ...
    def gauss_newton(self, fun, jac, x, args):
        ...
        while abs(np.linalg.norm(delta)) > 1e-4 and counter < 30:
            delta = np.linalg.pinv(jac(x, poses, img_positions)).dot(
                fun(x, poses, img_positions)
            )
            x = x - delta
            counter += 1
        return [x]
    ...
    def compute_pos_python(self, poses, img_positions, check_quality=False):
        ...
        res = opt.leastsq(
            self.residual, x, Dfun=self.residual_jac, args=(poses, img_positions)
        )  # scipy opt
...
def unroll_shutter(img_positions, poses, v, rot_rates, ecef_pos):
    ...
    errs = project(poses, ecef_pos) - project(
        poses + np.atleast_2d(dt).T.dot(np.atleast_2d(v)), ecef_pos
    )
