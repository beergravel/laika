...
from selfdrive.locationd.kalman.helpers import TEMPLATE_DIR, load_code, write_code
...
def sane(track):
    ...
    for i in range(1, len(diffs_x)):
        if (
            (diffs_x[i] > 0.05 or diffs_x[i - 1] > 0.05)
            and (diffs_x[i] > 2 * diffs_x[i - 1] or diffs_x[i] < 0.5 * diffs_x[i - 1])
        ) or (
            (diffs_y[i] > 0.05 or diffs_y[i - 1] > 0.05)
            and (diffs_y[i] > 2 * diffs_y[i - 1] or diffs_y[i] < 0.5 * diffs_y[i - 1])
        ):
            return False
    return True
...
class FeatureHandler:
    ...
    def __init__(self, K=5):
        ...
        def merge_features_c(tracks, features, empty_idxs):
            lib.merge_features(
                ffi.cast("double *", tracks.ctypes.data),
...
def generate_orient_error_jac(K):
    ...
    sympy_functions.append(
        (
            "orient_error_jac",
            img_pos_residual_sym.jacobian(dtheta),
