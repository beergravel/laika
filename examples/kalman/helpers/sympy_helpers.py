...
def euler_rotate(roll, pitch, yaw):
    ...
    matrix_roll = sp.Matrix(
        [[1, 0, 0], [0, sp.cos(roll), -sp.sin(roll)], [0, sp.sin(roll), sp.cos(roll)]]
    )
...
def sympy_into_c(sympy_functions, global_vars=None):
    ...
    for name, expr, args in sympy_functions:
        ...
        for aa in args:
            if aa is None:
                nargs.append(
                    codegen.InputArgument(sp.Symbol("unused"), dimensions=[1, 1])
                )
                continue
            found = False
    ...
    [(c_name, c_code), (h_name, c_header)] = codegen.get_code_generator(
        "C", "ekf", "C99"
    ).write(routines, "ekf")
