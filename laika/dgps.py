...
def mean_filter(delay):
    ...
    for i in range(max_step, len(delay) - max_step):
        ...
        if max_step in finite_idxs[0]:
            step = min(
                [max_step, finite_idxs[0][-1] - max_step, max_step - finite_idxs[0][0]]
            )
        ...
        for coord_file_path in coord_file_paths:
            try:
                ...
                for line_number in range(len(contents)):
                    ...
                    if (
                        not phase_center
                        and "ITRF2014 POSITION" in contents[line_number]
                    ):
                        velocity = [
...
def load_cors_station_positions(cache_dir):
    ...
    with open(cors_station_positions_path, "rb") as f:
        return np.load(
            f, allow_pickle=True
        ).item()  # pylint: disable=unexpected-keyword-arg


def get_station_position(
    station_id, cache_dir="/tmp/gnss/", time=GPSTime.from_datetime(datetime.utcnow())
):
    cors_station_positions_dict = load_cors_station_positions(cache_dir)
...
def parse_dgps(
    station_id,
    station_obs_file_path,
    dog,
    max_distance=100000,
    required_constellations=["GPS"],
):
    ...
    for prn in station_delays["C1C"]:
        if get_constellation(prn) == "GPS":
            ...
            for i in range(n):
                model_delays[prn][i] = dog.get_delay(
                    prn, times[i], station_pos, no_dgps=True
                )
    station_clock_errs = np.zeros(n)
    for i in range(n):
        station_clock_errs[i] = np.nanmean(
            [
                (station_delays["C1C"][prn][i] - model_delays[prn][i])
                for prn in model_delays
            ]
        )

    # remove clock errors and smooth out signal
    for prn in station_delays["C1C"]:
        station_delays["C1C"][prn] = mean_filter(
            station_delays["C1C"][prn] - station_clock_errs
        )
    for prn in station_delays["C2P"]:
        station_delays["C2P"][prn] = station_delays["C2P"][prn] - station_clock_errs

    return DGPSDelay(station_id, station_pos, station_delays, times, max_distance)


class DGPSDelay:
    def __init__(
        self, station_id, station_pos, station_delays, station_delays_t, max_distance
    ):
        self.id = station_id
    ...
    def get_delay(self, prn, time, signal="C1C"):
        ...
        if prn in self.delays[signal] and np.isfinite(
            self.delays[signal][prn][time_index]
        ):
    ...
    def valid(self, time, recv_pos):
        return (
            np.linalg.norm(recv_pos - self.pos) <= self.max_distance
            and time - self.delays_t[0] > -30
            and self.delays_t[-1] - time > -30
        )
