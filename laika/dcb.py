...
from .constants import SECS_IN_HR, SECS_IN_WEEK, SPEED_OF_LIGHT, GPS_L1, GPS_L2
...
class DCB:
    ...
    def get_delay(self, signal):
        ...
        if signal == "C2C":
            warnings.warn(
                "Differential code bias not implemented for signal C2C", UserWarning
            )
            return 0
        if signal == "C5C":
            warnings.warn(
                "Differential code bias not implemented for signal C5C", UserWarning
            )
            return 0
        if signal == "C6C":
            warnings.warn(
                "Differential code bias not implemented for signal C6C", UserWarning
            )
            return 0
        if signal == "C7C":
            warnings.warn(
                "Differential code bias not implemented for signal C7C", UserWarning
            )
            return 0
        if signal == "C8C":
            warnings.warn(
                "Differential code bias not implemented for signal C8C", UserWarning
            )
            return 0
...
def parse_dcbs(file_name, SUPPORTED_CONSTELLATIONS):
    ...
    for line in contents:
        ...
        epoch = (
            GPSTime.from_datetime(datetime.strptime(line_components[5], "%Y:%j:%f"))
            + 12 * SECS_IN_HR
        )
