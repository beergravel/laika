...
class ConstellationId(IntEnum):
    ...
    def from_qcom_source(cls, report_source: int):
        ...
        raise NotImplementedError(
            "Only GPS (0), GLONASS (1), BEIDOU (2) and SBAS (6) are supported from qcom, not:",
            {report_source},
        )
...
#     "GLONASS"
RINEX_ID_TO_CONSTELLATION: Dict[str, ConstellationId] = {
    id: con for con, id in RINEX_CONSTELLATION_TO_ID.items()
}
...
def get_closest(time, candidates, recv_pos=None):
    if recv_pos is None:
        ...
        return min(
            candidates, key=lambda candidate: abs(time - candidate.epoch), default=None
        )
...
def get_nmea_id_from_prn(prn: str):
    ...
    return get_nmea_id_from_constellation_and_svid(
        ConstellationId[constellation], sv_id
    )
...
def get_nmea_id_from_constellation_and_svid(constellation: ConstellationId, sv_id: int):
    ...
    raise ValueError(
        f"NMEA ID not found for constellation {constellation.name} with satellite id {sv_id}"
    )
