...
def compute_grid_lats_lons(data):
    ...
    for j, line in enumerate(data[1:]):
        if "LAT" in line:
            ...
            row_length = (
                lon2 - lon1
            ) / dlon + 1  # total number of values of longitudes
            next_lines_with_numbers = int(np.ceil(row_length / 16))
            elems_in_row = [
                min(16, int(row_length - i * 16))
                for i in range(next_lines_with_numbers)
            ]
...
class IonexMap:
    def __init__(self, exp, data1, data2):
        self.exp = exp
        self.t1 = GPSTime.from_datetime(
            dt.datetime(*[int(d) for d in data1[0].split()[:6]])
        )
        self.t2 = GPSTime.from_datetime(
            dt.datetime(*[int(d) for d in data2[0].split()[:6]])
        )
    ...
    def get_TEC(self, pos, time):
        ...
        (E00, E10), (E01, E11) = self.grid_TEC1[
        ...
        (E00, E10), (E01, E11) = self.grid_TEC2[
            lat_idxs[0] : lat_idxs[1] + 1, lon_idxs[0] : lon_idxs[1] + 1
        ]
        TEC_2 = (
            (1 - p) * (1 - q) * E00
        ...
        return (1 - (time - self.t1) / SECS_IN_HR) * TEC_1 + (
            (time - self.t1) / SECS_IN_HR
        ) * TEC_2
    ...
    def get_delay(self, rcv_pos, az, el, sat_pos, time, freq):
        ...
        slant_delay = vertical_delay * (
            (1 - ((EARTH_RADIUS * np.sin(beta)) / (EARTH_RADIUS + 3.5e5)) ** 2)
            ** (-0.5)
        )
        return slant_delay
...
def parse_ionex(ionex_file):
    ...
    if maps_count != len(map_start_idx):
        raise LookupError(
            "Parsing error: the number of maps in the header "
...
def klobuchar(pos, az, el, time, iono_coeffs):
    ...
    # ionospheric delay
    amp = iono_coeffs[0] + phi * (
        iono_coeffs[1] + phi * (iono_coeffs[2] + phi * iono_coeffs[3])
    )
    per = iono_coeffs[4] + phi * (
        iono_coeffs[5] + phi * (iono_coeffs[6] + phi * iono_coeffs[7])
    )
    if amp < 0.0:
        amp = 0.0
    if per < 72000.0:
