        ...
        if not os.path.isfile(filepath):
            ...
            try:
                ...
                raise DownloadFailed(
                    "Could not download file from: " + url_base + folder_path + filename
                )
            except (socket.timeout):
                raise DownloadFailed(
                    "Read timed out from: " + url_base + folder_path + filename
                )
...
def https_download_file(url):
    if "nasa.gov/" not in url:
        ...
        try:
            ...
            raise DownloadFailed(
                "Could not find .netrc file and no NASA_USERNAME and NASA_PASSWORD in environment for urs.earthdata.nasa.gov authentication"
            )
...
def download_and_cache_file_return_first_success(
    url_bases,
    ...
    for folder_path, filename in folder_and_file_names:
        try:
            file = download_and_cache_file(
                url_bases, folder_path, cache_dir, filename, compression, overwrite
            )
            return file
        except DownloadFailed as e:
            last_error = e
...
def download_and_cache_file(
    ...
    filepath = str(
        hatanaka.get_decompressed_path(os.path.join(folder_path_abs, filename))
    )
    ...
    if os.path.exists(filepath_attempt):
        ...
        if time.time() - last_attempt_time < SECS_IN_HR:
            raise DownloadFailed(
                f"Too soon to try downloading {folder_path + filename_zipped} from {url_base} again since last attempt"
            )
    if not os.path.isfile(filepath) or overwrite:
        try:
            ...
            raise DownloadFailed(
                f"Could not download {folder_path + filename_zipped} from {url_base} "
            )
    ...
    try:
        ...
        if GPSTime.from_datetime(datetime.utcnow()) - time > SECS_IN_DAY:
            ...
            return download_and_cache_file(
                url_bases, folder_path, cache_dir + "daily_nav/", filename, compression
            )
            ...
            return download_and_cache_file(
                url_base, folder_path, cache_dir + "hourly_nav/", filename, compression
            )
...
def download_orbits_gps(time, cache_dir, ephem_types):
    ...
    if (
        EphemerisType.FINAL_ORBIT in ephem_types
        and GPSTime.from_datetime(datetime.utcnow()) - time > 3 * SECS_IN_WEEK
    ):
    ...
    return download_and_cache_file_return_first_success(
        url_bases, folder_file_names, cache_dir + "cddis_products/", compression=".Z"
    )
...
def download_prediction_orbits_russia_src(gps_time, cache_dir):
    ...
    folder_and_file_names = [
        (folder_path, file_prefix + f"{h:02}.sp3") for h in reversed(current_day)
    ] + [
...
def download_orbits_russia_src(time, cache_dir, ephem_types):
    ...
    if (
        EphemerisType.FINAL_ORBIT in ephem_types
        and current_gps_time - time > 2 * SECS_IN_WEEK
    ):
    ...
    return download_and_cache_file_return_first_success(
        url_bases, folder_file_names, cache_dir + "russian_products/"
    )
...
def download_dcb(time, cache_dir):
    ...
    return download_and_cache_file_return_first_success(
        url_bases,
        list(zip(folder_paths, filenames)),
        cache_dir + "dcb/",
        compression=".gz",
        raise_error=True,
    )
...
def download_cors_coords(cache_dir):
    ...
    file_names = [
        file_name for file_name in file_names if file_name.endswith("coord.txt")
    ]
...
def download_cors_station(time, station_name, cache_dir):
    ...
    try:
        filepath = download_and_cache_file(
            url_bases, folder_path, cache_dir + "cors_obs/", filename, compression=".gz"
        )
        return filepath
    except DownloadFailed:
