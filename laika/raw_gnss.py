...
from .helpers import (
    ConstellationId,
    get_constellation_and_sv_id,
    get_nmea_id_from_constellation_and_svid,
...
def normal_meas_from_array(arr):
    ...
    return GNSSMeasurement(
        constellation_id, sv_id, arr[1], arr[2], observables, observables_std, arr[3]
    )
...
class GNSSMeasurement:
    ...
    def __init__(
        ...
        observables_std: Dict[str, float],
        glonass_freq: Union[int, float] = None,
    ):
        # Metadata
        # prn: unique satellite id
        self.prn = "%s%02d" % (
            constellation_id.to_rinex_char(),
            sv_id,
        )  # satellite ID in rinex convention
        ...
        self.sat_pos_final = np.array(
            [np.nan, np.nan, np.nan]
        )  # sat_pos in receiver time's ECEF frame instead of satellite time's ECEF frame
    ...
    def correct(self, est_pos, dog, allow_incomplete_delay=False):
        ...
        theta_1 = (
            constants.EARTH_ROTATION_RATE * geometric_range / constants.SPEED_OF_LIGHT
        )
        self.sat_pos_final = np.array(
            [
                self.sat_pos[0] * np.cos(theta_1) + self.sat_pos[1] * np.sin(theta_1),
                self.sat_pos[1] * np.cos(theta_1) - self.sat_pos[0] * np.sin(theta_1),
                self.sat_pos[2],
            ]
        )
        if "C1C" in self.observables_final and np.isfinite(
        ...
        if not self.corrected:
            if only_corrected:
                raise NotImplementedError(
                    "Only corrected measurements can be put into arrays"
                )
            else:
    ...
    def get_nmea_id(self):
        return get_nmea_id_from_constellation_and_svid(
            self.constellation_id, self.sv_id
        )


def process_measurements(
    measurements: List[GNSSMeasurement], dog
...
def correct_measurements(
    measurements: List[GNSSMeasurement], est_pos, dog, allow_incomplete_delay=False
) -> List[GNSSMeasurement]:
...
def read_raw_qcom(report):
    ...
    if constellation_id in [ConstellationId.GPS, ConstellationId.SBAS]:  # gps/sbas
        ...
        if dr:
            ...
            recv_time = GPSTime.from_glonass(
                report.glonassYear, report.glonassDay, recv_tow
            )
            time_bias_ms = report.glonassTimeBias
        else:
            recv_tow = report.milliseconds / 1000.0  # seconds
            recv_time = GPSTime.from_glonass(
                report.glonassCycleNumber, report.glonassNumberOfDays, recv_tow
            )
            time_bias_ms = report.timeBias
    else:
        raise NotImplementedError(
            "Only GPS (0), SBAS (1) and GLONASS (6) are supported from qcom, not:",
            {report.source},
        )
    ...
    for i in report.sv:
        ...
        if (
            not i.measurementStatus.measurementNotUsable
            and i.measurementStatus.satelliteTimeIsKnown
        ):
            sat_tow = (
                i.unfilteredMeasurementIntegral
            ...
            observables_std["C1C"] = (
                i.unfilteredTimeUncertainty * 1e-3 * constants.SPEED_OF_LIGHT
            )
            ...
            observables["S1C"] = (
                (i.carrierNoise / 100.0) if i.carrierNoise != 0 else np.nan
            )
            ...
            glonass_freq = (
                (i.glonassFrequencyIndex - 7)
                if constellation_id == ConstellationId.GLONASS
                else np.nan
            )
            measurements.append(
                GNSSMeasurement(
                ...
                if i.gnssId == ConstellationId.GLONASS:
                    ...
                    observables["D1C"] = (
                        -(
                            ...
                            / (
                                constants.GLONASS_L1
                                + glonass_freq * constants.GLONASS_L1_DELTA
                            )
                        )
                        * i.doppler
                    )
                    ...
                    glonass_freq = np.nan
                    observables["D1C"] = (
                        -(constants.SPEED_OF_LIGHT / constants.GPS_L1) * i.doppler
                    )
                observables_std["D1C"] = (
                    constants.SPEED_OF_LIGHT / constants.GPS_L1
                ) * i.dopplerStdev
    ...
    for i in range(n):
        ...
        for sat_str in obsdata_keys:
            ...
            for obs in obsdata.data[sat_str]:
                ...
                observables_std[rinex3_obs_key] = 1.0
...
def calc_pos_fix(
    measurements, x0=[0, 0, 0, 0, 0], no_weight=False, signal="C1C", min_measurements=6
):
    """
...
def calc_vel_fix(measurements, est_pos, v0=[0, 0, 0, 0], no_weight=False, signal="D1C"):
    ...
    Fx_vel = prr_residual(
        measurements, est_pos, signal=signal, no_weight=no_weight, no_nans=True
    )
...
def pr_residual(
    measurements: List[GNSSMeasurement], signal="C1C", no_weight=False, no_nans=False
):
    # solve for pos
    def Fx_pos(xxx_todo_changeme, no_weight=no_weight):
        ...
        for meas in measurements:
            if signal in meas.observables_final and np.isfinite(
                ...
                signal in meas.observables
                and np.isfinite(meas.observables[signal])
                and meas.processed
            ):
                ...
                theta = (
                    constants.EARTH_ROTATION_RATE * (pr - bc) / constants.SPEED_OF_LIGHT
                )
            else:
                if not no_nans:
                    rows.append(np.nan)
...
def prr_residual(measurements, est_pos, signal="D1C", no_weight=False, no_nans=False):
    ...
    def Fx_vel(vel, no_weight=no_weight):
        rows = []
        for meas in measurements:
            if signal not in meas.observables or not np.isfinite(
            ...
            los_vector = (sat_pos - est_pos[0:3]) / np.linalg.norm(
                sat_pos - est_pos[0:3]
            )
            rows.append(
                weight
                * (
                    (meas.sat_vel - vel[0:3]).dot(los_vector)
...
def get_Q(recv_pos, sat_positions):
    ...
    A = np.column_stack(
        (
            sat_positions_rel[:, 0]
            / sat_distances,  # pylint: disable=unsubscriptable-object
            sat_positions_rel[:, 1]
            / sat_distances,  # pylint: disable=unsubscriptable-object
            sat_positions_rel[:, 2]
            / sat_distances,  # pylint: disable=unsubscriptable-object
