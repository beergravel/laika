...
from .ephemeris import (
    ...
    PolyEphemeris,
    parse_sp3_orbits,
    parse_rinex_nav_msg_gps,
    parse_rinex_nav_msg_glonass,
)
from .downloader import (
    download_orbits_gps,
    download_orbits_russia_src,
    download_nav,
    download_ionex,
    download_dcb,
    download_prediction_orbits_russia_src,
)
...
class AstroDog:
    ...
    def __init__(
        ...
        self.pull_orbit = (
            len(set(EphemerisType.all_orbits()) & set(valid_ephem_types)) > 0
        )
        ...
        self.nav: DefaultDict[
            str, List[Union[GPSEphemeris, GLONASSEphemeris]]
        ] = defaultdict(list)
        self.dcbs: DefaultDict[str, List[DCB]] = defaultdict(list)

        self.cached_ionex: Optional[IonexMap] = None
        self.cached_dgps = None
        self.cached_orbit: DefaultDict[str, Optional[PolyEphemeris]] = defaultdict(
            lambda: None
        )
        self.cached_nav: DefaultDict[
            str, Union[GPSEphemeris, GLONASSEphemeris, None]
        ] = defaultdict(lambda: None)
        self.cached_dcb: DefaultDict[str, Optional[DCB]] = defaultdict(lambda: None)

    def get_ionex(self, time) -> Optional[IonexMap]:
        ionex: Optional[IonexMap] = self._get_latest_valid_data(
            self.ionex_maps, self.cached_ionex, self.get_ionex_data, time
        )
        if ionex is None:
            if self.auto_update:
                raise RuntimeError(
                    "Pulled ionex, but still can't get valid for time " + str(time)
                )
        else:
    ...
    def get_nav(self, prn, time):
        ...
        nav = self._get_latest_valid_data(
            self.nav[prn], self.cached_nav[prn], self.get_nav_data, time, skip_download
        )
        if nav is not None:
    ...
    def get_orbit(self, prn: str, time: GPSTime):
        ...
        orbit = self._get_latest_valid_data(
            self.orbits[prn],
            self.cached_orbit[prn],
            self.get_orbit_data,
            time,
            skip_download,
        )
    ...
    def get_orbits(self, time):
        ...
        return AstroDog._select_valid_temporal_items(
            self.orbits, time, self.cached_orbit
        )

    def get_dcb(self, prn, time):
        skip_download = time in self.dcbs_fetched_times
        dcb = self._get_latest_valid_data(
            self.dcbs[prn], self.cached_dcb[prn], self.get_dcb_data, time, skip_download
        )
        if dcb is not None:
    ...
    def get_dgps_corrections(self, time, recv_pos):
        ...
        if latest_data is None:
            if self.auto_update:
                raise RuntimeError(
                    "Pulled dgps, but still can't get valid for time " + str(time)
                )
        else:
    ...
    def _add_ephems(
        self, new_ephems: Dict[str, List[Ephemeris]], ephems_dict, fetched_times
    ):
    ...
    def get_nav_data(self, time):
        def download_and_parse(constellation, parse_rinex_nav_func):
            file_path = download_nav(
                time, cache_dir=self.cache_dir, constellation=constellation
            )
        ...
        if "GPS" in self.valid_const:
            fetched_ephems = download_and_parse(
                ConstellationId.GPS, parse_rinex_nav_msg_gps
            )
        if "GLONASS" in self.valid_const:
            for k, v in download_and_parse(
                ConstellationId.GLONASS, parse_rinex_nav_msg_glonass
            ).items():
    ...
    def download_parse_orbit(
        self, gps_time: GPSTime, skip_before_epoch=None
    ) -> Dict[str, List[PolyEphemeris]]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if "GPS" in self.valid_const:
                futures_gps = [
                    executor.submit(
                        download_orbits_gps, t, self.cache_dir, self.valid_ephem_types
                    )
                    for t in time_steps
                ]

            ephems_other = parse_sp3_orbits(
                [f.result() for f in futures_other if f.result()],
                self.valid_const,
                skip_before_epoch,
            )
            ephems_us = (
                parse_sp3_orbits(
                    [f.result() for f in futures_gps if f.result()],
                    self.valid_const,
                    skip_before_epoch,
                )
                if futures_gps
        ...
        return {
            k: ephems_other.get(k, []) + ephems_us.get(k, [])
            for k in set(list(ephems_other.keys()) + list(ephems_us.keys()))
        }
    ...
    def download_parse_prediction_orbit(self, gps_time: GPSTime):
        ...
        if result is not None:
            ...
            result = [
                download_orbits_gps(t, self.cache_dir, self.valid_ephem_types)
                for t in [gps_time - SECS_IN_DAY, gps_time]
            ]
        if result is None:
            return {}
        return parse_sp3_orbits(
            result, self.valid_const, skip_until_epoch=skip_until_epoch
        )
    ...
    def get_orbit_data(self, time: GPSTime, only_predictions=False):
        ...
        if sum([len(v) for v in ephems_sp3.values()]) < 5:
            raise RuntimeError(
                f"No orbit data found. For Time {time.as_datetime()} constellations {self.valid_const} valid ephem types {self.valid_ephem_types}"
            )
    ...
    def get_dgps_data(self, time, recv_pos):
        station_names = get_closest_station_names(
            recv_pos, k=8, max_distance=MAX_DGPS_DISTANCE, cache_dir=self.cache_dir
        )
        for station_name in station_names:
            file_path_station = download_cors_station(
                time, station_name, cache_dir=self.cache_dir
            )
            if file_path_station:
                dgps = parse_dgps(
                    ...
                    file_path_station,
                    self,
                    max_distance=MAX_DGPS_DISTANCE,
    ...
    def get_frequency(self, prn, time, signal="C1C"):
        if get_constellation(prn) == "GPS":
            ...
            if n is None:
                return None
            switch = {
                "1": constants.GLONASS_L1 + n * constants.GLONASS_L1_DELTA,
    ...
    def get_delay(self, prn, time, rcv_pos, no_dgps=False, signal="C1C", freq=None):
        ...
        if self.auto_update and (ionex is None or dcb is None or freq is None):
            return None
        iono_delay = (
            ionex.get_delay(rcv_pos, az, el, sat_pos, time, freq)
            if ionex is not None
            else 0.0
        )
        trop_delay = saast(rcv_pos, el)
        code_bias = dcb.get_delay(signal) if dcb is not None else 0.0
