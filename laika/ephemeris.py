...
class Ephemeris(ABC):
    def __init__(
        ...
        data,
        epoch: GPSTime,
        eph_type: EphemerisType,
        healthy: bool,
        max_time_diff: float,
        file_epoch: Optional[GPSTime] = None,
        file_name=None,
    ):
        self.prn = prn
        self.data = data
        ...
        self.file_source = (
            "" if file_name is None else file_name.split("/")[-1][:3]
        )  # File source for the ephemeris (e.g. igu, igr, Sta)
        self.__json = None
...
class PolyEphemeris(Ephemeris):
    ...
    def _get_sat_info(self, time: GPSTime):
        ...
        time_err = sum(
            (dt**p) * self.data["clock"][deg_t - p] for p in range(deg_t + 1)
        )
        time_err_rate = sum(
            p * (dt ** (p - 1)) * self.data["clock"][deg_t - p]
            for p in range(1, deg_t + 1)
        )
        time_err_with_rel = (
            time_err - 2 * np.inner(sat_pos, sat_vel) / SPEED_OF_LIGHT**2
        )
...
class GPSEphemeris(Ephemeris):
    ...
    def _get_sat_info(self, time: GPSTime):
        ...
        tdiff = (
            time - eph["toe"]
        )  # Time of ephemeris (might be different from time of clock)

        # Calculate position per IS-GPS-200D p 97 Table 20-IV
        a = eph["sqrta"] * eph["sqrta"]  # [m] Semi-major axis
        ma_dot = (
            sqrt(EARTH_GM / (a * a * a)) + eph["dn"]
        )  # [rad/sec] Corrected mean motion
        ...
        cal_dot = al_dot * (
            1.0 + 2.0 * (eph["cus"] * cos(2.0 * al) - eph["cuc"] * sin(2.0 * al))
        )

        # Calculate corrected radius based on argument of latitude
        r = a * tempd1 + eph["crc"] * cos(2.0 * al) + eph["crs"] * sin(2.0 * al)
        r_dot = a * eph["ecc"] * sin(ea) * ea_dot + 2.0 * al_dot * (
            eph["crs"] * cos(2.0 * al) - eph["crc"] * sin(2.0 * al)
        )
        ...
        inc_dot = eph["inc_dot"] + 2.0 * al_dot * (
            eph["cis"] * cos(2.0 * al) - eph["cic"] * sin(2.0 * al)
        )
...
def parse_sp3_orbits(
    file_names, supported_constellations, skip_until_epoch: Optional[GPSTime] = None
            ...
            while True:
                ...
                # pos line
                    ...
                    if len(data[prn]) < 1 or epoch - data[prn][-1][1] > 0:
                        parsed = [
                            (ephem_type, file_epoch, file_name),
                            epoch,
                            1e3 * float(line[4:18]),
...
def read_prn_data(data, prn, deg=16, deg_t=1):
    ...
    if len(np_data_prn) == 0 or (np_data_prn[:, 5] > 0.99).any():
        return []
    ephems = []
    for i in range(len(np_data_prn) - deg):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter(
                "ignore"
            )  # Ignores: UserWarning: The value of the smallest subnormal for <class 'numpy.float64'> type is zero.
            poly_data["xyz"] = poly.polyfit(
                times, measurements[:, 1:].astype(float), deg
            )
        poly_data["clock"] = [
            (np_data_prn[epoch_index + 1][5] - np_data_prn[epoch_index - 1][5]) / 1800,
            np_data_prn[epoch_index][5],
        ]
        poly_data["deg"] = deg
        ...
        ephems.append(
            PolyEphemeris(
                prn, poly_data, epoch, ephem_type, file_epoch, file_name, healthy=True
            )
        )
    return ephems
...
def parse_rinex_nav_msg_gps(file_name):
    ...
    while True:
        ...
        if rinex_ver == 3:
            ...
            epoch = GPSTime.from_datetime(
                datetime.strptime(epoch_str, "%y %m %d %H %M %S")
            )
            line = " " + line  # Shift 1 char to the right
...
def parse_qcom_ephem(qcom_poly, current_week):
    ...
    poly_data["xyz"] = np.array(
        [
            ...
            [data.xyz0[2], data.xyzN[6], data.xyzN[7], data.xyzN[8]],
        ]
    ).T
